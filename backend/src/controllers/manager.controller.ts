...
export async function createVerification(req: Request, res: Response) {
  try {
    ...
    const verifiedPersons = await Promise.all(
      persons.map(async (p: { idType: GovIdType; idNumber: string }) => {
        const record = await VerificationService.verifyIdentity(p.idType, p.idNumber);
        if (record) {
          return {
            name: record.name,
            idType: p.idType,
          ...
          allVerified = false;
          return {
            name: "", // Do not return placeholder or "Unknown"
            idType: p.idType,
            idNumber: p.idNumber,
            verified: false,
            response: {
              source: "registry",
              message: "Invalid ID / No record found in GovIdRecord",
              idType: p.idType,
              idNumber: p.idNumber,
            },
          };
    ...
    if (!allVerified) {
      res.status(200).json({
        ...
        verified: false,
        error: "Invalid ID / No record found in GovIdRecord",
        verification: {
          type,
          verified: false,
    ...
    // Attach overall verified: true flag for frontend consistency
    const resultVerification = {
      ...verification,
      verified: true,
    };

    res.status(201).json({ message: "Verification created", verified: true, verification: resultVerification });
  } catch (err) {
